...
export default function Header() {
  ...
  <div>
      ...
      <button onClick={() => {setPagesOpen(!Pagesopen); setBlogOpen(false); setShopOpen(false);}} className="text-white text-[20px] flex items-center"> Pages</button>
      {Pagesopen && (<div className="absolute left-1 mt-3 w-[200px] bg-white text-black rounded-xl shadow-xl p-3 space-y-2 ">
          <p onClick={() => router.push("/aboutUs")}>About Us</p>  <p>Chef</p>  <p> Chef Details</p>  <p>Login</p>  </div>
           ...
           <div className="relative">
      <button onClick={() => {setBlogOpen(!Blogopen); setPagesOpen(false); setShopOpen(false);} }className="text-white text-[20px] flex items-center"> Blog</button>
      {Blogopen && (<div className="absolute left-1 mt-3 w-[200px] bg-white text-black rounded-xl  p-3 space-y-2">
          <p onClick={() => router.push("/blog")}>Blog Standard</p>  <p>Blog with sidebar</p>  <p> Blog with two column</p>   </div>
            ...
            <div className="relative">
      <button onClick={() =>{ setShopOpen(!Shopsopen); setPagesOpen(false); setBlogOpen(false);} }className="text-white text-[20px] flex items-center"> Shop</button>
...
<img className="hidden md:flex h-[50px] w-[50px] md:ml-[100px]" src="https://media.licdn.com/dms/image/v2/C560BAQEo2fBEZzQT3g/company-logo_100_100/company-logo_100_100/0/1630652388207?e=2147483647&v=beta&t=gdM6BRE4x-HDBpJ9b8i7nXbXuVemb0JTALwF83sDF0s"/></div>
<div className=" md:h-[50px] w-[85px] md:w-[200px] mb-4 bg-black border-2 flex items-center border-white hover:bg-white ">
  <span className="text-white text-[20px] md:p-12 hover:text-black" onClick={() => router.push("/contactUs")}
        >Reservation</span>
</div></div>
</div>
      </div>
